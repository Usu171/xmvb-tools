...
def process_GTO(part, mol):
    gto_dict = {
        number: [[basis_type, line] for basis_type, line in read_basis_parts(part1)]
...
def ReadMolden(filename):
    ...
    d5_ao_number = sum(basis[0] * 2 + 1 for atom in mol['GTO'].values() for basis in atom)
    d6_ao_number = sum(
...
def WriteAtoms(file, Atoms):
    ...
    for number, atom in Atoms.items():
        ...
        coords_str = ' '.join(f'{coord:>17.10f}' for coord in coordinate)
        file.write(f'{element:>3} {number:>3} {charge_str} {coords_str}\n')
...
def WriteGTO(file, GTO):
    ...
    for number, basis_list in GTO.items():
        ...
        for basis_type, basis_line in basis_list:
            file.write(f' {reversed_basis_dict[basis_type].lower()} {len(basis_line)} 1.0\n')
            for basis in basis_line:
                file.write(f'  {basis[0]:>15.10f} {basis[1]:>15.17f}\n')
        file.write('\n')
